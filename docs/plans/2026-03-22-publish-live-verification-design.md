...
- 早期本地读取时观测到的外部边界为 `80 x 127`
- 进入 publish source trim 后，真正参与 write plan、写入与 comparison 的 canonical 数据集为 `58 x 127`

当前验证线的 live verification、write plan 与 comparison 口径都以前者为边界观察、以后者为实际运行矩阵。
...
真实样本 `全国执行` 的早期观测边界为 `80 x 127`；但进入 publish source trim 后，真正参与写入与 comparison 的 canonical 数据集为 `58 x 127`。由于列数仍然超过飞书单次 `100` 列限制，因此当前设计中的列感知分段仍然成立。
